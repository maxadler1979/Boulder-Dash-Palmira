...
/*
 * Boulder Dash — Апогей БК-01 (КР580ВМ80А + ВГ75/ВТ57).
 *
 * Карта (видео НИЖЕ мониторных ячеек E100–E1CF, иначе мерцание):
 *   0000 – …      код / BSS (не заходить за SM_Y)
 *   стек          с SM_Y вниз
 *   SM_Y          таблица смещений VIDEO_ROWS слов
 *   SCREEN..E0FF  видеобуфер VIDEO_LEN
 *   E100–E1CF     переменные монитора — НЕ ТРОГАТЬ
 *   E1D0–EAFF     штатное видео монитора (не используем)
 *   EC00          ВИ53 звук
 *   ED00          ВВ55 клавиатура
 *   EE00          ВВ55 пользовательский
 *   EF00          ВГ75
 *   F000          ВТ57 (запись) / ПЗУ (чтение)
 *
 * Видео: ВГ75 считает VG75_TIMING_ROWS строк; в ОЗУ VIDEO_ROWS×78,
 * последний байт EOS (F3h).
 */
...
/* Базы периферии Апогей */
#define APOGEE_VG75   0xEF00
#define APOGEE_VT57   0xF000
#define APOGEE_KBD    0xED00
#define APOGEE_VI53   0xEC00

/* SCREEN = 0xE100 - VIDEO_LEN; SM_Y = SCREEN - VIDEO_ROWS*2 */
#define SCREEN    0xD668
#define SM_Y_ADDR 0xD620
#define HUD_Y     3
#define FIELD_Y   4
...
#define VSYNC_HZ       50   /* Апогей БК01: 50 Гц (Пальмира ~59–60) */
...
extern unsigned char quota_jingle_phase;
extern unsigned char quota_jingle_sub;
