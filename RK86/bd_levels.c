...
void move_camera_right(void)
{
    if (sdvig < 4)
    {
        if (map_x < MAP_X_MAX)      // не увеличиваем sdvig на границе — некуда рендерить доп. спрайт
        {
            sdvig += 2;
        }
    }
...
void calculateWindowCenter(void)
{
    ...
    // ������� ����
    uint8_t windowWidth = WIN_W;
    uint8_t windowHeight = WIN_H;
